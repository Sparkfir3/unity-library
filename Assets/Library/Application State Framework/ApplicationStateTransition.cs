...
namespace AppStateSystem {
    ...
    public class ApplicationStateTransition : MonoBehaviour {
        ...
        [SerializeField, Tooltip("Controls how state changes are handled during the transition")]
        ...
        public List<IEnumerator> CoroutinesPreTransition { get; private set; } = new List<IEnumerator>();
        public List<IEnumerator> CoroutinesMidTransition { get; private set; } = new List<IEnumerator>();
        public List<IEnumerator> CoroutinesPostTransition { get; private set; } = new List<IEnumerator>();

        // ---

        [TitleGroup("Scene Loading Settings"), SerializeField, Tooltip("Whether all the scenes to load should be added onto existing scenes")]
...
#region Misc
        ...
        private IEnumerator RunListOfIEnumerators(List<IEnumerator> list) {
            foreach(IEnumerator iEnum in list) {
                yield return ApplicationStateManager.Instance.StartCoroutine(iEnum);
            }
        }
